...
public class EdgeHealthMonitorTask extends TimerTask {
	...
	public void run() {
		...
		/*
		 * While checking the health of inbound edges I check if node is present
		 * in outbound edge because my assumption is if a heart beat is received
		 * from node which is present in outbound edge then I consider it as
		 * reply. This assumption will cause inbound heartbeat not updated in
		 * cyclic loop but channel is still active. This is the only use case,
		 * and remaining all other use cases are handled.
		 */
		edgeMonitor.getInboundEdges().getEdgesMap().values().stream()
			.filter(ei -> ((currentTime - ei.getLastHeartbeat()) > (edgeMonitor
				.getDelayTime()) && !edgeMonitor.getOutboundEdges().hasNode(ei.getRef())))
			.forEach(ei -> {
		...
		edgeMonitor.getOutboundEdges().getEdgesMap().values().stream()
			.filter(ei -> (currentTime
				- ei.getLastHeartbeat()) > (edgeMonitor.getDelayTime() * 2))
			.forEach(ei -> {
				long hb = currentTime - ei.getLastHeartbeat();
				if (debug)
					edgeMonitor.getLogger()
						.info("Last heat beat reply received before: " + hb);
				ei.setChannel(null);
				ei.setActive(false);
			});
	}
