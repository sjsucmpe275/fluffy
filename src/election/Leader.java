...
public class Leader implements INodeState, FollowerListener {
	...
	/*
	 * For this event I just reply with SIZEIS message. Since term is set when
	 * CANDIDATE starts election, this leader will go to Follower state as part
	 * of request vote event.
	 *
	 */
	@Override
	public void handleGetClusterSize(WorkMessage workMessage, Channel channel) {
		 ...
		 * .getOutboundEdges().getEdgesMap(); for (Integer destinationId :
		 * edgeMap.keySet()) { EdgeInfo edge = edgeMap.get(destinationId); if
		 * (edge.isActive() && edge.getChannel() != null) {
	...
	/*
	 * In this state we don't need implementation for this event or message
	 * type.
	 */
	@Override
	...
	public void handleLeaderIs(WorkMessage workMessage, Channel channel) {
		...
		/*
		 * If there is another node which is leader in new term, then I update
		 * myself and go back to election state
		 */
		...
		/*
		 * If there is another leader which is elected in same term then I go
		 * back to candidate state to start new election This scenario might
		 * happen because we dont have constant cluster size and we evaluate
		 * dynamically and it need not be that in 2 seconds we get entire
		 * cluster size and split votes might occur, to be on the safe side its
		 * better to go to CANDIDATE
		 */
	...
	/*
	 * If Vote Request is for term higher than the term I am leader in then I
	 * should vote and move back to follower
	 */
	@Override
	...
	/*
	 * There might be a delay to receive a vote from far neighbour, It is ok,
	 * not to consider a vote once I become a leader.
	 */
	@Override
	...
	/*
	 * Check for the term message came from, i if it is higher than my term then
	 * go to follower state, if equal then go to candidate state. if lesser then
	 * update beat time from follower
	 */
	@Override
	public void handleBeat(WorkMessage workMessage, Channel channel) {
		...
		followerMonitor.onBeat(followerId, System.currentTimeMillis()); // notify
																		// follower
																		// monitor
																		// about
																		// heart
