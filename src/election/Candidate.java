...
public class Candidate implements INodeState, TimeoutListener {
	...
	private Timer timer;
	// I need this to identify size of cluster
	...
	public Candidate(ServerState state) {
		...
		this.timer = new Timer(this, state.getConf().getElectionTimeout(),
			"Cand - Election Timer");
	...
	public void handleVoteResponse(WorkMessage workMessage, Channel channel) {
		...
		// getLeaderId - nothing other than candiate Id, I set this value before
		// I send vote request
		 
		if (leader.getVotedFor() == state.getConf().getNodeId()
	...
	/**
	 * Term is more than or equal my term I will become follower, if lesser
	 * don't do anything
	 */
	@Override
	...
	public void afterStateChange() {
		...
		clear();
		getClusterSize(); // Broadcasts GETCLUSTERSIZE message and starts a
							// timer
	}

	@Override
	public void notifyTimeout() {
		logger.info("~~~~~~~~Candidate - Notify Timeout Event - "
		...
		timer.start(() -> {
			...
			if (myVoteCount > requiredVotes || (myVoteCount == 1 && requiredVotes == 1)) {
				
				// Update myself as a Leader in the new term and broad cast
				// LEADERIS message
				state.setElectionId(state.getElectionId() + 1);
				state.setLeaderId(state.getConf().getNodeId());

				// TODO this should be in separate method.
				state.getEmon().broadcastMessage(util.createLeaderIsMessage(state));
	...
	private void getClusterSize() {
		state.getEmon().broadcastMessage(util.createGetClusterSizeMessage(state, -1));
		timer.start();
	}
