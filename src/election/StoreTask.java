...
public class StoreTask extends AbstractTask implements Runnable {
	...
	public StoreTask(ServerState state, ITaskListener listener, WorkMessage requestMsg,
		IReplicationStrategy replicationStrategy, List<Integer> availableNodes) {
		super(state, listener, requestMsg);
		this.replicationStrategy = replicationStrategy;
		this.availableNodes = availableNodes;
		this.size = requestMsg.getTask().getTaskMessage().getQuery().getMetadata()
			.getSeqSize();
	}
	...
	protected void cleanup() {
		...
		try {
			/*
			 * Here I goto sleep for some time to ensure I don't send redundant
			 * messages over the network. This Sleep is not required, if each
			 * node know its distance from Leader and update its max hops based
			 * on that.. In the current solution this is the tradeoff I have
			 */
